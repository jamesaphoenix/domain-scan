...
fn test_php_interface_names() {
    ...
    assert!(
        names.contains(&"UserRepositoryInterface"),
        "Missing UserRepositoryInterface"
    );
    assert!(
        names.contains(&"NotificationServiceInterface"),
        "Missing NotificationServiceInterface"
    );
...
fn test_php_interface_methods() {
    ...
    let user_repo = ir
        ...
        .unwrap();
    assert_eq!(
        user_repo.methods.len(),
        3,
        "UserRepositoryInterface should have 3 methods"
    );
}
...
fn test_php_interface_extends() {
    ...
    let notif = ir
        ...
        .unwrap();
    assert!(
        !notif.extends.is_empty(),
        "NotificationServiceInterface should extend ServiceInterface"
    );
}
...
fn test_php_class_extends() {
    ...
    let order = ir
        ...
        .unwrap();
    assert_eq!(
        order.extends.as_deref(),
        Some("BaseController"),
        "OrderController should extend BaseController"
    );
}
...
fn test_php_class_implements() {
    ...
    assert!(
        user_service
            .implements
            .contains(&"UserRepositoryInterface".to_string()),
...
fn test_php_class_methods() {
    ...
    assert!(
        user_service.methods.len() >= 5,
        "UserService should have at least 5 methods, got {}",
...
fn test_php_class_properties() {
    ...
    let user_service = ir.classes.iter().find(|c| c.name == "UserService").unwrap();
    assert!(
        user_service.properties.len() >= 2,
        "UserService should have at least 2 properties"
    );
}
...
fn test_php_trait_names() {
    ...
    let trait_names: Vec<&str> = ir
        ...
        .collect();
    assert!(
        trait_names.contains(&"Timestampable"),
        "Missing Timestampable"
    );
    assert!(
        trait_names.contains(&"SoftDeletable"),
        "Missing SoftDeletable"
    );
}
...
fn test_php_trait_methods() {
    let ir = extract_fixture("traits.php");
    let timestampable = ir
        .interfaces
        .iter()
        .find(|i| i.name == "Timestampable")
        .unwrap();
    assert_eq!(
        timestampable.methods.len(),
        2,
        "Timestampable should have 2 methods"
    );
}
...
fn test_php_import_sources() {
    ...
    assert!(
        sources.iter().any(|s| s.contains("User")),
        "Missing User import"
    );
    assert!(
        sources.iter().any(|s| s.contains("Request")),
