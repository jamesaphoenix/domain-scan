...
fn test_python_class_methods() {
    ...
    assert_eq!(
        user_svc.methods.len(),
        3,
        "Expected 3 methods (excluding __init__)"
    );
...
fn test_python_protocols() {
    ...
    assert!(
        ir.interfaces.len() >= 3,
        "Expected at least 3 protocols, got {}",
        ir.interfaces.len()
    );
}
...
fn test_python_abstract_classes() {
    ...
    assert!(
        ir.interfaces.len() >= 2,
        "Expected at least 2 ABC classes, got {}",
        ir.interfaces.len()
    );
}
...
fn test_python_import_from_symbols() {
    let ir = extract_fixture("imports.py");
    let typing_import = ir
        .imports
        .iter()
        .find(|i| i.source == "typing")
        .expect("typing import not found");
    let symbol_names: Vec<&str> = typing_import
        .symbols
        .iter()
        .map(|s| s.name.as_str())
        .collect();
...
fn test_python_services_fastapi() {
    ...
    assert!(
        ir.services.len() >= 4,
        "Expected at least 4 FastAPI services, got {}",
        ir.services.len()
    );
}
